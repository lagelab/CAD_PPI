...
paths = list.files('data/genoppi_input/run030', full.names = T)  # old: paths = list.files('data/genoppi_input/run006', full.names = T) 
...
#write.table(extract_ip_accession(list.files('~/Projects/03_MICOM/data/genoppi_input/run030/', full.names = T)), '201111_micom_accession_numbers.tsv', quote = F, row.names = F)
uniprot_to_gene <- read.delim(url('https://www.uniprot.org/mapping/M20201111A94466D2655679D1FD8953E075198DA80FFADAQ.tab')) # generated by extracting accession numbers from all files an quering with uniprot (see above line).
...
dfs1 = lapply(paths[paths.whitehead], function(path) {
  ...
  newpath = gsub('run030', 'run031', path)
