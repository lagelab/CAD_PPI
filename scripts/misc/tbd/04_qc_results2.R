...
{
  myfit <- limma::lmFit(subset(df, select = -c(gene)), method = "robust", maxit = 5000)
...
# translate bait to a fixed name
path_bait_table <- read.csv('~/Projects/03_MICOM/data/pathname_bait_to_gene.csv', stringsAsFactors = F)
all_baits = unique(c(path_bait_table$To, 'PLPP3', 'PPAP2B', 'KIAA1462'))
...
for (path in paths){
  ...
  # check data
  df = read.csv(path, sep = '\t', stringsAsFactors = F) 
  newpath = gsub(targetrun, writerun, path)
  ...
  path.bait.direct = unlist(lapply(strsplit(path,'\\.|(vs)'), function(x) x[3]))
  ...
  path.vs = unlist(lapply(strsplit(path,'\\.|(vs)'), function(x) x[4]))
  #path.vs[path.vs == 'Whitehead'] <- NA
  path.cell = unlist(lapply(strsplit(path, '\\.'), function(x) x[4]))
  path.date = unlist(lapply(strsplit(path, '\\.'), function(x) x[6]))
  path.facility = ifelse(grepl('Broad', path), 'Broad', 'Whitehead')
  
  #print(paste0(path.bait,' (', path.bait.direct, ')...'))
  count = count + 1
  out = paste0(path.bait.direct,'|',path.bait,'|',path.bait.inweb, ' in ', path.cell, ' (',path.facility,')')
  write(paste0('[',count,'] ',path), stdout())
  write(out, stdout())
  ...
  ip.bait.found <- path.bait %in% dfstat$gene | ip.flag.tag.found | path.bait.inweb %in% dfstat$gene
  ip.bait.enriched <- ifelse(path.bait %in% dfstat$gene[dfstat$significant], T, F) | ifelse(path.bait.inweb %in% dfstat$gene[dfstat$significant], T, F) 
  ...
  rowsummary <- data.frame(date = path.date, path = path, npath = newpath, 
