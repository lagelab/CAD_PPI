...
library(genoppi) # v1.0.13 (for InWeb, BioPlex, iRefIndex data)
library(RCX) # v1.2.2 (for readCX function to read Cytoscape CX format data)
...
huri_genes <- read.csv('../data/PPI_Databases/HuRI_Luck2020_SuppTable2.txt',
	header=T,sep='\t',stringsAsFactors=F)
huri_genes$ENSG <- sapply(strsplit(huri_genes$ensembl_gene_id,'\\.'),'[[',1)

huri_ints <- read.csv('../data/PPI_Databases/HuRI_Luck2020_SuppTable11.txt',
	header=T,sep='\t',stringsAsFactors=F)
...
string_genes <- read.table(
	'../data/PPI_Databases/STRING_9606.protein.info.v11.5.short.txt.gz',
	header=F,sep='\t',stringsAsFactors=F)
names(string_genes) <- c('protein_id','gene_name','protein_size')

string_ints <- read.table(
	'../data/PPI_Databases/STRING_9606.protein.physical.links.v11.5.txt.gz',
	header=T,sep=' ',stringsAsFactors=F)
...
# NOTE: input file below not included in GitHub due to size
rcx <- readCX('../data/PPI_Databases/PCNet_Huang2018.cx')
...
# name of PPI datasets (BAIT_CELLTYPE)
...
# iterate through each PPI dataset
for (prefix in prefixes) {
	prefixTable <- subset(intDf,ListName==prefix)
	bait <- strsplit(prefix,'_')[[1]][1]
	
	# annotate if interactors found in databases
	tempDf <- subset(prefixTable,Interactor)[,c("CellType","Bait","GeneName")]
	dbListDf <- NULL # store data for overlap enrichment test below
...
write.table(overlapRes,'../output/CAD_DatabaseIntsOverlapStats.txt',
	quote=F,row.names=F,sep='\t')


# generate combined EC, SMC, EC-SMC pie charts of reported vs. novel PPIs
# according to each PPI database
pieDf <- rbind(intAnnDf,data.frame(CellType='EC-SMC',unique(intAnnDf[,-1]))) |>
	...
	count(value) |>
	mutate(perc = 100* n / sum(n)) # calculate percentages

pdf('../output/CAD_DatabaseIntsPieCharts.pdf',width=7,height=3)
...
write.table(pieDf,'../output/CAD_DatabaseIntsPieChartStats.txt',
	quote=F,row.names=F,sep='\t')
...
write.table(intAnnDf,'../output/CAD_DatabaseIntsAnnotations.txt',
	quote=F,row.names=F,sep='\t')
